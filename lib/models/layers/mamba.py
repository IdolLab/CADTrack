...
class Cross_Mamba_Attention_SSM(nn.Module):
    ...
    def forward(self, x_rgb: torch.Tensor, x_e: torch.Tensor):
        ...
        # y_rgb = selective_scan(
        #     x_rgb, dt_rgb,
        #     A_rgb_e, B_e, C_rgb, self.D_1.float(),
        #     delta_bias=self.dt_proj_1.bias.float(),
        #     delta_softplus=True,
        # )
        # y_e = selective_scan(
        #     x_e, dt_e,
        #     A_rgb_e, B_rgb, C_e, self.D_2.float(),
        #     delta_bias=self.dt_proj_2.bias.float(),
        #     delta_softplus=True,
        # )
        # assert out_y.dtype == torch.float
...
class MA_SS2D(nn.Module):
    ...
    def __init__(
        ...
        self.Mamba_Attention_SSM = Mamba_Attention_SSM(
            d_model=self.d_model,
            d_state=self.d_state,
            ssm_ratio=ssm_ratio,
            dt_rank=dt_rank,
    ...
    def forward(self, x_rgb: torch.Tensor):
        ...
        if self.d_conv > 1:
            x_rgb_trans = x_rgb.permute(0, 2, 1).contiguous()
            x_rgb_conv = self.act(self.conv1d(x_rgb_trans))
            x_rgb_conv = rearrange(x_rgb_conv, "b d hw -> b hw d")
            y_rgb = self.Mamba_Attention_SSM(x_rgb_conv)
            y_rgb = y_rgb.view(B, HW, -1)
            y_rgb = y_rgb * y_rgb_act
        out_rgb = self.dropout_rgb(self.out_proj_rgb(y_rgb))
        ...
        return out_rgb
# =====================================================
class Mamba_Attention_SSM(nn.Module):
    def __init__(
            self,
            # basic dims ===========
            d_model=96,
